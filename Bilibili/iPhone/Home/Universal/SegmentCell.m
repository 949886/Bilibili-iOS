...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_type == SegmentCellRecommend && _data)
    {
        ...
        videoCell.titleLabel.text = element.title;
        [videoCell.imageView sd_setImageWithURL:[NSURL URLWithString:element.cover] placeholderImage:[UIImage imageWithColor:[UIColor lightGrayColor]]];

        if ([_recSegment.type isEqualToString:@"region"] ||
            [_recSegment.type isEqualToString:@"recommend"])
        {
            videoCell.playCountLabel.text = [NSString stringWithFormat:@"%@", [NSString integer2Chinese:element.play]];
            videoCell.danmakuCountLabel.text = [NSString stringWithFormat:@"%ld", (long)element.danmaku];
        }
        
        //若为活动等内容，隐藏播放数和弹幕数。
        if ([_recSegment.type isEqualToString:@"activity"] ||
            [_recSegment.type isEqualToString:@"bangumi"])
            [videoCell hideCountData];
        else [videoCell showCountData];;
    }
    ...
    if (_type == SegmentCellBangumi && _data)
    {
        ...
        if (_bangumiSegment.index != 2)
        {
            ...
            bangumiCell.titleLabel.text = bangumi.title;
            bangumiCell.episodeLabel.text = [NSString stringWithFormat:NSLocalizedString(@"home_bangumi_currentEpisode", nil), bangumi.newest_ep_index];
            
            [bangumiCell.coverImageView sd_setImageWithURL:[NSURL URLWithString:bangumi.cover] placeholderImage:[UIImage imageWithColor:[UIColor lightGrayColor]]];
