...
-(void)setType:(SegmentViewType)type
{
    ...
    switch (type)
    {
        ...
        {
            NSString * nibName = IS_IPAD ? @"IPadLiveCell" : @"IPhoneLiveCell";
            UINib * nib = [UINib nibWithNibName:nibName bundle:nil];
            [self.collectionView registerNib:nib forCellWithReuseIdentifier:ID];
        } break;
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_type == SegmentViewLive && _data)
    {
        ...
        liveCell.descriptionLabel.text = live.title;
        liveCell.viewsLabel.text = [NSString stringWithFormat:@"%ld", (long)live.online];
