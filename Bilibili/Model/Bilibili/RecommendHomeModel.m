...
@implementation RecommendHomeModel
...
{
    return @{@"segments" : [RecommendSegment class]};
}

-(void)setSegments:(NSArray<RecommendSegment *> *)segments
{
    //不知为何服务器偶尔会返回空数据，需要删除 || type == av 会返回bilibili周刊，不在首页显示，需要删除
    NSMutableArray * array = [NSMutableArray array];
    for (RecommendSegment * segment in segments)
...
@implementation RecommendSegment

+(NSDictionary *)modelCustomPropertyMapper
{
    return @{@"live_count" : @"ext.live_count",
             @"elements" : @"body",
             @"banners" : @[@"banner.top", @"banner.bottom"]};
}

+(NSDictionary *)modelContainerPropertyGenericClass
{
    return @{@"elements" : [RecommendElement class],
...
@implementation RecommendElement
